...
export default function TrofaPage() {
  ...
  return (
    <main className="bg-transparent text-white overflow-x-hidden font-sans relative pb-20">
      ...
      {/* 導入セクション（幕に被らないように左右にpadding） */}
      <section className="min-h-[80vh] flex flex-col items-center justify-center relative px-[80px] md:px-[180px] text-center pt-32">
        <div className="text-[#d4af37] text-xl md:text-2xl mb-4 tracking-[0.3em]">★ THE ATTRACTIONS ★</div>
        <h1 className="text-4xl md:text-8xl font-black text-[#d4af37] tracking-widest font-serif drop-shadow-lg mb-8">
          trofaの魔法
        </h1>
        <p className="text-sm md:text-2xl text-gray-300 font-medium tracking-widest max-w-2xl leading-relaxed mb-20">
      ...
      {/* z-10なので、z-50の幕の後ろを通り抜けるように見えます */}
      <section className="relative w-full flex flex-col items-center z-10 shadow-[0_0_50px_rgba(0,0,0,0.8)]">
        ...
        <div 
          ...
          className="w-full flex items-center overflow-x-auto snap-x snap-mandatory py-10 relative bg-[#0a0203] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] px-[60px] md:px-[160px]"
        ...
        >
          {extendedFeatures.map((f, i) => (
            <div key={i} className="flex-shrink-0 snap-center flex items-center">
              ...
              {/* カード本体（フィルムの1コマ） */}
              <div className="w-[75vw] md:w-[45vw] bg-[#1a0508] border border-[#d4af37]/30 rounded-xl shadow-2xl flex flex-col p-4 md:p-6 mx-2">
                ...
                <div className="w-full text-center mt-6">
                  ...
                  <h3 className="text-lg md:text-3xl font-bold mb-3 font-serif text-white">{f.title}</h3>
                  <p className="text-gray-400 text-xs md:text-base leading-relaxed px-2">{f.description}</p>
                </div>
              </div>
